...
public class ColorPickerDialog extends Dialog implements
    ...
    public ColorPickerDialog(Context context, int theme, int initialColor,
            int androidColor, int CustomColor) {
        super(context, theme);
        mInitialColor = initialColor;
        mAndroidColor = androidColor;
        mCustomColor = CustomColor;
        if (mAndroidColor != 0x00000000 && mCustomColor != 0x00000000) {
    ...
    public boolean onMenuItemClick(MenuItem item) {
        if (item.getItemId() == R.id.palette_custom) {
            mPalette = PALETTE_CUSTOM;
            mColorTransitionAnimator.start();
            return true;
            ...
            return true;
        } else if (item.getItemId() == R.id.reset_custom) {
            mColorPicker.setColor(mCustomColor, true);
            return true;
        }
        return false;
    }
